...
if(isset ($_SESSION['username']) && $_SESSION['level']==2){
	...
	$row = mysql_fetch_array(mysql_query("select address,phone from company where id='".$_SESSION['username']."'"));
...
工作類型* : <select name="work_type" id="work_type"><option>請選擇</option></select> 
			<select name="work_type_list" id="work_type_list"></select><!-- 要等 work_type 選完才載入 -->
			<span id="work_type_hint"></span>
			<br> 

截止日期* : <select name="year" id="year"></select>年
			<select name="month" id="month"></select>月
			<select name="date" id="date"></select>日&nbsp;
			<select name="hour" id="hour"></select>時 
			<select name="minute" id="minute"></select>分<br>
...
<script>
	...
	$(function(){
		...
		$("#year").append($("<option></option>").attr("value", year_array[i]).text(year_array[i]));
		//生成 月
		for(var i=1;i<=12;i++)
		...
		// 生成 時
		for(var i=0;i<=23;i++)
		$("#hour").append($("<option></option>").attr("value", i).text(i));
		// 生成 分
		for(var i=0;i<=59;i++)
		$("#minute").append($("<option></option>").attr("value", i).text(i));
		...
		$("#month").change(function() {
			...
			for(var i=1;i<=d;i++) $("#date").append($("<option>").attr("value", i).text(i));
		});
		// 工作類型改變時，用ajax列出工作類型細目
		$('#work_type').change(function() {
			var id = $(this).val();
			// 清空工作類別細目
			$("#work_type_list option").remove();
			// 執行AJAX取得細目資料
			$.ajax({
			type:"POST",
			async:false, 
			...
			data:"id="+id,
			success:function(msg){ $('#work_type_list').html(msg);	},
